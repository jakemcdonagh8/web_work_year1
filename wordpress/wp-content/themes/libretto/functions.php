 ...
 */
function libretto_password_form() {
	global $post;
	$label = esc_attr__( 'pwbox-'.( empty( $post->ID ) ? rand() : $post->ID ) );
	$password_form = esc_html__( 'This post is password-protected. To read it, please enter the password below.', 'libretto' ) .
		'<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" method="post"><label for="' . $label . '">' . __( 'Password:', 'libretto' ) . ' </label><input title="' . __( 'Password', 'libretto' ) .'" placeholder="' . __( 'Password', 'libretto' ) .'"  name="' . esc_attr__( 'password', 'libretto' ) .'" id="' . $label . '" type="password" size="20" maxlength="20" /><p class="form-submit"><input type="submit"  name="' . __( 'submit', 'libretto' ) .'" value="' . esc_attr__( 'Submit', 'libretto' ) . '" /></p>
    </form>';
	return $password_form;
}
add_filter( 'the_password_form', 'libretto_password_form' );
